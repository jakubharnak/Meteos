...
temt6000 = ADC(Pin(35), atten = ADC.ATTN_11DB)
...
#-udaje pre mqtt----------------------------------------------------------------
klient = "153"#153
ip = "meteos.ddns.net"#192.168.1.48
idc = 0
user = "h"
password = "h"
ssl = False
...
try:
    ...
    text = (
        "0 - OK",
        "1 - nepodporovaná verzia protokolu",
        "2 - identifikátor zamietnutý",
        "3 - nedostupný server",
        "4 - nesprávne meno alebo heslo",
        "5 - chybná autorizácia",
        "6 - spojenie sa stratilo"
    )
    chyba = e.args[0]
    if (chyba in range(0, len(text))):
        chyba = text[chyba]    
    print("\x1b[91mMQTT broker {} oznámil chybu {}\x1b[0m".format(mqtt.server, chyba))
except:
    print("\x1b[91mChyba spojenia s MQTT brokerom {}!\x1b[0m".format(mqtt.server))
